...
class AzurePRFetcher:
    ...
    async def fetch_file_changes(self, pull_request_id: int) -> List[FileChange]:
        async with httpx.AsyncClient(timeout=self.config.request_timeout_seconds) as client:
            try:
                ...
                raise
            source_commit_id = (
                (pr_data.get("lastMergeSourceCommit") or {}).get("commitId") or ""
            )
            target_commit_id = (
                (pr_data.get("lastMergeTargetCommit") or {}).get("commitId") or ""
            )
            ...
            for entry in changes:
                ...
                base_content = ""
                if source_commit_id:
                    target_content = await self._get_file_content_at_commit(client, path, source_commit_id)
                if target_commit_id:
                    base_content = await self._get_file_content_at_commit(client, path, target_commit_id)
                file_changes.append(
                    FileChange(
                        path=path,
