...
async def load_rule_chunks() -> List[RuleChunk]:
    config = load_config()
    rules_dir = Path(config.rules.rules_path)
    rules_dir.mkdir(parents=True, exist_ok=True)
    pdf_text = load_all_pdfs(rules_dir)
    txt_text = load_all_txt(rules_dir)
    chunks: List[RuleChunk] = []
    if pdf_text.strip():
        chunks.extend(chunk_rules(pdf_text, source="pdf", max_chars=1500))
    if txt_text.strip():
        chunks.extend(chunk_rules(txt_text, source="txt", max_chars=1500))
    return chunks
...
async def review_pull_request(
    ...
    config = load_config()
    fetcher = AzurePRFetcher(repository=repository)
    commenter = AzurePRCommenter(repository=repository) if post_comments else None
    rule_agent = RuleAgent()
    expert_agent = ExpertAgent()
    critic_agent = CriticAgent()
    rule_chunks = await load_rule_chunks()
    prepared_chunks = prepare_chunks_for_store(rule_chunks)
    retriever = RuleRetriever()
    await retriever.build_index_if_needed(prepared_chunks)
    file_changes = await fetcher.fetch_file_changes(pull_request_id)
    ...
    if post_comments and commenter is not None:
        ...
        await commenter.post_summary_comment(pull_request_id, summary)
    ...
    if post_comments and commenter is not None:
        for violation in rule_violations:
            ...
            await commenter.post_inline_comment(pull_request_id, file_path, line_number, content)
        for key in [
            "bugs",
            "performance_issues",
            "security_issues",
